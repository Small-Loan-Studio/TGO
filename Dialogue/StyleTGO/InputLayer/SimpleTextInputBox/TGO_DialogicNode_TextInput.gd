...
@export_node_path var cancel_button: NodePath
...
func _ready() -> void:
	...
	if cancel_button:
		get_node(cancel_button).pressed.connect(_on_cancel_button_pressed)
	if input_line_edit:
...
func _on_confirmation_button_pressed(text: String = "") -> void:
	if get_node(input_line_edit) is LineEdit:
...
func _on_cancel_button_pressed() -> void:
	get_node(input_line_edit).text = ""
	DialogicUtil.autoload().TextInput.input_confirmed.emit("")
